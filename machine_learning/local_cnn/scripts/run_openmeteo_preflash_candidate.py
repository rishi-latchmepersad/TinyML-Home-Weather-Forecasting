...
def main() -> int:
    ...
    candidate_output_directory = project_root / "artifacts" / "openmeteo_preflash_candidate"
    staging_directory = candidate_output_directory / "staging"
    tuner_directory = project_root / "keras_tuner_dir" / "openmeteo_preflash_candidate"
    ...
    recent_local_config = PipelineConfig(
        ...
        tuner_directory=project_root / "keras_tuner_dir" / "openmeteo_preflash_candidate_recent_unused",
    ...
    print("Building broad Open-Meteo training dataset...", flush=True)
    ...
    print("Running broad tuning/fine-tuning stage...", flush=True)
    ...
    print("Building recent local adaptation dataset...", flush=True)
    ...
    print("Running recent local adaptation stage...", flush=True)
    ...
    print("Running pruning/quantization...", flush=True)
    ...
    final_model_path = candidate_output_directory / "best_pruned_model.keras"
    pruning.best_pruned_model.save(final_model_path)
    final_scaler_path = _save_scaler(recent_dataset, candidate_output_directory / "feature_scaler.json")
    _copy_if_present(quantization.float32_tflite_path, candidate_output_directory / "pruned_float32.tflite")
    _copy_if_present(quantization.int8_tflite_path, candidate_output_directory / "pruned_int8.tflite")
    ...
    summary = {
        ...
        "quantization": {
            ...
            "tflite_fp32_val_mae": None if quantization.tflite_fp32_val_mae is None else float(quantization.tflite_fp32_val_mae),
            "tflite_fp32_test_mae": None if quantization.tflite_fp32_test_mae is None else float(quantization.tflite_fp32_test_mae),
            "tflite_int8_val_mae": None if quantization.tflite_int8_val_mae is None else float(quantization.tflite_int8_val_mae),
            "tflite_int8_test_mae": None if quantization.tflite_int8_test_mae is None else float(quantization.tflite_int8_test_mae),
            "float32_size_bytes": None if quantization.float32_size_bytes is None else int(quantization.float32_size_bytes),
            "int8_size_bytes": None if quantization.int8_size_bytes is None else int(quantization.int8_size_bytes),
            "int8_weight_bytes": None if quantization.int8_weight_bytes is None else int(quantization.int8_weight_bytes),
        },
        "artifacts": {
            "best_pruned_model": str(final_model_path),
            "feature_scaler": str(final_scaler_path),
            "pruned_float32_tflite": str(candidate_output_directory / "pruned_float32.tflite"),
            "pruned_int8_tflite": str(candidate_output_directory / "pruned_int8.tflite"),
    ...
    summary_path = candidate_output_directory / "training_summary.json"
