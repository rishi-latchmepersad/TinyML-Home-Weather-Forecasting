...
#define LOGGER_TASK_STACK_WORDS (4096u)
#endif
...
#endif
#ifndef LOGGER_MAX_CONSECUTIVE_ERRORS
#define LOGGER_MAX_CONSECUTIVE_ERRORS (5u) /* reboot only after repeated failures */
#endif
...
#endif
#ifndef LOGGER_CLOSE_FILE_WHEN_IDLE
/*
 * Keep the daily CSV closed whenever we are not actively flushing data so
 * reader tasks (for example the baseline forecaster) can open the latest file.
 */
#define LOGGER_CLOSE_FILE_WHEN_IDLE 1
#endif
...
__attribute__((section(".sram1"), aligned(32))) static char g_write_buffer[LOGGER_BUFFER_BYTES];
...
static void measurement_logger_close_active_file_if_open(void);
...
bool measurement_logger_task_create(void)
{
	...
	if (g_logger_task_handle == NULL)
	{
		const osThreadAttr_t attr = {
			.name = "sd_card_logger_task",
			.priority =
				(osPriority_t)LOGGER_TASK_PRIORITY,
			.stack_size =
...
static void measurement_logger_task_entry(void *argument)
{
	...
	while (1)
	{
		...
		case LOGGER_STATE_RUNNING:
		{
			...
			if (g_sync_requested || ((now - last_flush_tick) >= pdMS_TO_TICKS(LOGGER_FLUSH_PERIOD_MS)))
			{
				state = LOGGER_STATE_FLUSH;
			}
			break;
...
//                                        }
		...
		case LOGGER_STATE_ERROR_BACKOFF:
			...
			printf(LOG_PREFIX "Error in the measurement logger (attempt %lu/%lu). Retrying in %lu ms.\r\n",
				   (unsigned long)consecutive_error_count,
				   (unsigned long)LOGGER_MAX_CONSECUTIVE_ERRORS,
				   (unsigned long)LOGGER_BACKOFF_MS);
			...
			if (consecutive_error_count >= LOGGER_MAX_CONSECUTIVE_ERRORS)
			{
				error_handler_with_message("Measurement logger failed repeatedly (10 attempts). Rebooting.\n");
			}
...
static bool measurement_logger_flush_buffer_to_file(void)
{
		...
		else if (fr == FR_NO_FILE)
		{
			...
			if (fr == FR_OK)
			{
				if (f_size(&g_active_file) == 0)
				{
					...
					UINT bw = 0;
					if (f_write(&g_active_file, hdr, (UINT)strlen(hdr), &bw) == FR_OK && bw == (UINT)strlen(hdr))
					{
	...
	g_write_used = 0u;
#if LOGGER_CLOSE_FILE_WHEN_IDLE
	measurement_logger_close_active_file_if_open();
#endif
	return true;
}
