...
#define FORECAST_TEMP_TASK_STACK_WORDS      (2048u)
...
#else
// The deployed RNN should contribute 5% of the final forecast.
#define FORECAST_TEMP_BASELINE_BLEND_ALPHA_SHORT  (0.05f)
#define FORECAST_TEMP_BASELINE_BLEND_ALPHA_LONG   (0.05f)
#endif
...
typedef struct {
	...
	// Track the running sum of humidity readings inside this slot.
	float humidity_sum;
	// Track the running sum of pressure readings inside this slot.
	float pressure_sum;
...
static float g_slot_temperature_history[FORECAST_TEMP_HISTORY_CAPACITY] =
		{ 0.0f };
...
static void forecast_temp_bootstrap_reset_bundle(
...
static void forecast_temp_bootstrap_submit_bundle(
		forecast_temp_measurement_bundle_t *bundle,
		forecast_temp_slot_bucket_t *slot_bucket, uint32_t *minute_counter,
		uint32_t *slot_counter);
// Forward declare a helper that accumulates a bundle into the current slot bucket.
static void forecast_temp_bootstrap_accumulate_bundle(
		const forecast_temp_measurement_bundle_t *bundle,
		forecast_temp_slot_bucket_t *slot_bucket, uint32_t *minute_counter,
		uint32_t *slot_counter);
// Forward declare a helper that finalizes the current slot bucket into features.
static void forecast_temp_bootstrap_finalize_slot(
		forecast_temp_slot_bucket_t *bucket, uint32_t *slot_counter);
// Forward declare a helper that streams one CSV file into the bootstrap machinery.
static void forecast_temp_bootstrap_process_file(const char *file_path,
		forecast_temp_slot_bucket_t *slot_bucket, uint32_t *minute_counter,
		uint32_t *slot_counter);
...
bool forecast_temp_task_start(void) {
	...
	const osThreadAttr_t thread_attributes = { .name = "ForecastTemp",
			.priority = FORECAST_TEMP_TASK_PRIORITY, .stack_size =
					(uint32_t) (FORECAST_TEMP_TASK_STACK_WORDS
							* sizeof(StackType_t)) };
	// Create the mutex that protects the shared prediction output.
	if (g_prediction_mutex == NULL) {
		const osMutexAttr_t prediction_mutex_attributes = { .name =
				"forecast_temp_prediction_mutex", .attr_bits = osMutexRecursive
				| osMutexPrioInherit, .cb_mem = NULL, .cb_size = 0u };
		g_prediction_mutex = osMutexNew(&prediction_mutex_attributes);
	}
	// Start the FreeRTOS task using the CMSIS wrapper.
	g_forecast_thread_id = osThreadNew(forecast_temp_task_entry, NULL,
			&thread_attributes);
...
static void forecast_temp_bootstrap_accumulate_bundle(
	...
	if (!(bundle->has_temperature && bundle->has_humidity
	...
	if ((slot_bucket->sample_count > 0u)
			&& (strncmp(slot_bucket->slot_key, bundle_slot_key,
					sizeof(slot_bucket->slot_key)) != 0)) {
		// Finalize the existing bucket so the new slot can start fresh.
		forecast_temp_bootstrap_finalize_slot(slot_bucket, slot_counter);
	}
	// Initialize the slot key whenever this is the first sample for the bucket.
	if (slot_bucket->sample_count == 0u) {
		// Copy the bundle slot key into the bucket so future comparisons work.
		(void) strncpy(slot_bucket->slot_key, bundle_slot_key,
				sizeof(slot_bucket->slot_key) - 1u);
		// Guarantee the stored slot key is null-terminated.
		slot_bucket->slot_key[sizeof(slot_bucket->slot_key) - 1u] = '\0';
	}
	// Add the bundle temperature into the running slot sum.
	slot_bucket->temperature_sum += bundle->temperature_c;
	// Add the bundle humidity into the running slot sum.
	slot_bucket->humidity_sum += bundle->humidity_pct;
	// Add the bundle pressure into the running slot sum.
...
static void forecast_temp_bootstrap_finalize_slot(
	...
	// Derive the average temperature across the slot.
	const float slot_temperature = bucket->temperature_sum * reciprocal;
	// Derive the average humidity across the slot.
	const float slot_humidity = bucket->humidity_sum * reciprocal;
	// Derive the average pressure across the slot.
	...
	// Increment the slot counter when the caller supplied storage for it.
	if (slot_counter != NULL) {
		*slot_counter += 1u;
	}
	// Clear the slot key so the next accumulation knows it is starting fresh.
	bucket->slot_key[0] = '\0';
...
static void forecast_temp_bootstrap_submit_bundle(
	...
	if (bundle->has_temperature && bundle->has_humidity && bundle->has_pressure
			&& bundle->has_illuminance) {
		// Hand the bundle to the slot accumulator so the sums stay current.
		forecast_temp_bootstrap_accumulate_bundle(bundle, slot_bucket,
...
static void forecast_temp_bootstrap_process_file(const char *file_path,
	...
	if ((file_path == NULL) || (slot_bucket == NULL)) {
		return;
	}
	...
	while (1) {
		...
		if ((timestamp == NULL) || (sensor == NULL) || (quantity == NULL)
				|| (value_str == NULL)) {
			continue;
		}
		...
		if ((strcmp(sensor, "bme280") == 0)
				&& (strcmp(quantity, "temperature_c") == 0)) {
		...
		} else if ((strcmp(sensor, "bme280") == 0)
				&& (strcmp(quantity, "humidity_pct") == 0)) {
		...
		} else if ((strcmp(sensor, "veml7700") == 0)
				&& (strcmp(quantity, "lux_lx") == 0)) {
...
static void forecast_temp_bootstrap_from_sd_card(void) {
	...
	while (1) {
		...
		const char *name_ptr =
				(info.fname[0] != '\0') ? info.fname : info.altname;
		...
		if (sscanf(name_ptr, "measurements_%4u-%2u-%2u.csv", &year, &month,
				&day) != 3) {
			continue;
		}
		...
		if (file_count < FORECAST_TEMP_BOOTSTRAP_MAX_FILES) {
			...
			(void) strncpy(files[file_count].filename, name_ptr,
					sizeof(files[file_count].filename) - 1u);
			// Guarantee the stored filename is null-terminated.
			files[file_count].filename[sizeof(files[file_count].filename) - 1u] =
					'\0';
			...
			if (date_key > oldest_key) {
				(void) strncpy(files[oldest_index].filename, name_ptr,
						sizeof(files[oldest_index].filename) - 1u);
				files[oldest_index].filename[sizeof(files[oldest_index].filename)
						- 1u] = '\0';
	...
	// Flush any partial slot that may still be sitting in the accumulator.
	forecast_temp_bootstrap_finalize_slot(&slot_bucket, &slot_counter);
	...
	if (slot_counter > 0u) {
		...
		printf(
				"[forecast] bootstrap did not assemble any complete 30-minute slots\r\n");
	}
}
...
static void forecast_temp_task_entry(void *argument) {
	...
	while (1) {
		...
		case FORECAST_TEMP_STATE_WAIT_MINUTE:
			...
			vTaskDelayUntil(&context.last_wake_tick,
		...
		case FORECAST_TEMP_STATE_FETCH_SENSORS: {
			...
			const bool have_bme = bme280_get_latest(&context.temperature_c,
		...
		case FORECAST_TEMP_STATE_CHECK_SLOT: {
			...
			if (!have_full_slot) {
				...
				printf(
						"[forecast] waiting for full 30-minute slot (%lu/%lu minute samples)\r\n",
		...
		case FORECAST_TEMP_STATE_APPEND_FEATURES:
			...
			if (g_feature_window_count < FORECAST_TEMP_WINDOW_LENGTH) {
				printf(
						"[forecast] Not enough filled slots for the complete window. We have %d out of %d",
						g_feature_window_count, FORECAST_TEMP_WINDOW_LENGTH);
				state = FORECAST_TEMP_STATE_WAIT_MINUTE;
		...
		case FORECAST_TEMP_STATE_EXPORT_WINDOW:
			...
			if (!forecast_temp_export_window_to_network(
					g_network_input_buffer)) {
				state = FORECAST_TEMP_STATE_WAIT_MINUTE;
			} else {
...
static bool forecast_temp_initialize_network(void) {
	...
	ai_handle *activations_table =
			AI_FORECAST_TEMP_ML_MODEL_DATA_ACTIVATIONS_TABLE_GET();
	...
	ai_error err = ai_forecast_temp_ml_model_create_and_init(
	...
	forecast_temp_extract_quantization(&g_network_inputs[0], &g_input_scale,
			&g_input_zero_point);
...
static void forecast_temp_store_slot_history(float temperature_c,
	...
	if (g_slot_history_count >= FORECAST_TEMP_HISTORY_CAPACITY) {
		g_slot_history_head = (g_slot_history_head + 1u)
				% FORECAST_TEMP_HISTORY_CAPACITY;
		g_slot_history_count -= 1u;
	}
	// Compute the index where the new sample should be written.
	const size_t tail_index = (g_slot_history_head + g_slot_history_count)
			% FORECAST_TEMP_HISTORY_CAPACITY;
...
static void forecast_temp_append_feature_vector(
	...
	if (g_feature_window_count >= FORECAST_TEMP_WINDOW_LENGTH) {
		g_feature_window_head = (g_feature_window_head + 1u)
				% FORECAST_TEMP_WINDOW_LENGTH;
		g_feature_window_count -= 1u;
	}
	// Compute where the new feature vector should be written.
	const size_t tail_index = (g_feature_window_head + g_feature_window_count)
			% FORECAST_TEMP_WINDOW_LENGTH;
	// Copy the feature vector into the ring buffer.
	memcpy(g_feature_window[tail_index], normalized_features,
			sizeof(float) * FORECAST_TEMP_FEATURE_COUNT);
...
static bool forecast_temp_export_window_to_network(ai_u8 *destination_buffer) {
	...
	for (size_t time_index = 0u;
		...
		if (time_index < FORECAST_TEMP_WINDOW_LENGTH) {
			source_index = (g_feature_window_head + time_index)
					% FORECAST_TEMP_WINDOW_LENGTH;
		} else {
			...
			const size_t last_valid_index = (g_feature_window_head
					+ FORECAST_TEMP_WINDOW_LENGTH - 1u)
					% FORECAST_TEMP_WINDOW_LENGTH;
