...
static inline uint8_t bcd_to_bin(uint8_t b) { return (uint8_t)(((b >> 4) * 10u) + (b & 0x0Fu)); }
static inline uint8_t bin_to_bcd(uint8_t d) { return (uint8_t)(((d / 10u) << 4) | (d % 10u)); }
...
HAL_StatusTypeDef ds3231_read_time_iso8601_utc_i2c1(char *dst, size_t cap)
{
    ...
    HAL_StatusTypeDef st =
        HAL_I2C_Master_Transmit(&hi2c1, DS3231_ADDR_8BIT, &reg, 1u, 50u);
    ...
    (void)snprintf(dst, cap, "%04u-%02u-%02uT%02u:%02u:%02uZ",
                   year, month, day, hours24, mins, secs);
    return HAL_OK;
}
                                                           ...
                                                           uint8_t second_0_59)
{
    ...
    frame[7] = bin_to_bcd((uint8_t)((year_yyyy >= 2000u) ? (year_yyyy - 2000u) : 0u));

    return HAL_I2C_Master_Transmit(&hi2c1, DS3231_ADDR_8BIT, frame, sizeof frame, 100u);
}
...
void i2c1_scan_and_log_devices(void)
{
    ...
    for (uint8_t addr7 = 0x08u; addr7 < 0x78u; ++addr7) {
        uint8_t dummy = 0;
        if (HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)(addr7 << 1), &dummy, 0u, 5u) == HAL_OK) {
