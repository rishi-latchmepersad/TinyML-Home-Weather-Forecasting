...
static uint8_t SD_SendCommand(uint8_t cmd, uint32_t arg) {
	...
	if (cmd == SD_CMD12)
		SD_SPI_ReadByte(); /* Skip a stuff byte when stop reading */
...
DSTATUS USER_initialize(BYTE pdrv /* Physical drive nmuber to identify the drive */
	...
	if (SD_SendCommand(SD_CMD0, 0) == 1) { /* Enter Idle state */
		...
		if (SD_SendCommand(SD_CMD8, 0x1AA) == 1) { /* SDv2? */
			...
			for (n = 0; n < 4; n++)
				ocr[n] = SD_SPI_ReadByte();

			if (ocr[2] == 0x01 && ocr[3] == 0xAA) { /* The card can work at vdd range of 2.7-3.6V */
				/* Wait for leaving idle state (ACMD41 with HCS bit) */
				while ((HAL_GetTick() < timeout)
						&& SD_SendCommand(SD_CMD41 | 0x80, 1UL << 30))
					;

				if ((HAL_GetTick() < timeout)
						&& SD_SendCommand(SD_CMD58, 0) == 0) { /* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++)
						ocr[n] = SD_SPI_ReadByte();
...
UINT count /* Number of sectors to read */
	...
	if (count == 1) { /* Single block read */
		if ((SD_SendCommand(SD_CMD17, sector) == 0)
				&& SD_ReadDataBlock(buff, 512))
			count = 0;
	} else { /* Multiple block read */
		/* Fallback: perform multi-sector read as repeated single-block reads.
		 This avoids CMD18/STOP edge cases that can stall f_getfree(). */
		do {
			/* Send CMD17 and read one 512B sector */
			if (SD_SendCommand(SD_CMD17, sector) != 0x00
					|| !SD_ReadDataBlock(buff, 512)) {
				SD_CS_HIGH();
				(void) SD_SPI_ReadByte(); // trailing dummy clock after deselect
				return RES_ERROR;
			}
			sector++;
			buff += 512;
...
UINT count /* Number of sectors to write */
	...
	if (count == 1) { /* Single block write */
		if ((SD_SendCommand(SD_CMD24, sector) == 0)
				&& SD_WriteDataBlock(buff, 0xFE))
			count = 0;
	} else { /* Multiple block write */
		if (CardType & CT_SDC)
			SD_SendCommand(SD_CMD23 | 0x80, count);
		if (SD_SendCommand(SD_CMD25, sector) == 0) {
			do {
				if (!SD_WriteDataBlock(buff, 0xFC))
					break;
				buff += 512;
			} while (--count);
...
void *buff /* Buffer to send/receive control data */
	...
	case CTRL_SYNC: /* Make sure that no pending write process */
		SD_CS_LOW();
		(void) SD_SPI_ReadByte();             // prime the clock before polling
		if (SD_WaitReady())
			res = RES_OK;    // wait (bounded by SD_TIMEOUT)
		SD_CS_HIGH();
		(void) SD_SPI_ReadByte();        // one dummy after deselect releases DO
		break;

	case GET_SECTOR_COUNT: /* Get number of sectors on the disk (DWORD) */
		if ((SD_SendCommand(SD_CMD9, 0) == 0) && SD_ReadDataBlock(csd, 16)) {
			if ((csd[0] >> 6) == 1) { /* SDC ver 2.00 */
				cs = csd[9] + ((DWORD) csd[8] << 8)
						+ ((DWORD) (csd[7] & 63) << 16) + 1;
				*(DWORD*) buff = cs << 10;
			} else { /* SDC ver 1.XX or MMC */
				n = (csd[5] & 15) + ((csd[10] & 128) >> 7) + ((csd[9] & 3) << 1)
						+ 2;
				cs = (csd[8] >> 6) + ((DWORD) csd[7] << 2)
						+ ((DWORD) (csd[6] & 3) << 10) + 1;
				*(DWORD*) buff = cs << (n - 9);
			}
			res = RES_OK;
